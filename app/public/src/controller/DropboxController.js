class DropboxController {
    constructor() {
        ...
        this.onselectionchange = new Event('selectionchange'); //everytime that a file is selected, the selection change is shot

        this.btnSendFileEl = document.querySelector('#btn-send-file');
        this.inputFilesEl = document.querySelector('#files');
        this.snakeModalEl = document.querySelector('#react-snackbar-root');
        this.progressBarEl = this.snakeModalEl.querySelector('.mc-progress-bar-fg'); //progress bar is inside the snakemodal
    ...
    //Each uploaded file has an specific li. Inside the li, the data of the file is set as a string in "dataset"
    getSelection() {
        return this.listFilesEl.querySelectorAll('.selected');
    }

    //a promise for each file we are going to delete
    removeTask() {
    ...
    initEvents() {
        ...
        this.btnRename.addEventListener('click', e => {
            ...
            let file = JSON.parse(li.dataset.file); //for all data inside the li element  
            ...
            if (name) {
                ...
                this.getFirebaseRef().child(li.dataset.key).set(file);  //finds a child with the same key of the li and saves the file with the new name in the firebase
        ...
        this.btnSendFileEl.addEventListener('click', event => {
            this.inputFilesEl.click();     //open the tab that annexes files

        });

        this.inputFilesEl.addEventListener('change', event => {

            // console.log(event.target.files); //the target of the element that was hitten 
            ...
            this.uploadTask(event.target.files).then(responses => {
                ...
                responses.forEach(resp => {
                    ...
                    //a collection, it means we can push 
                    this.getFirebaseRef().push().set(resp.files['input-file']); //it means files.input-file
    ...
    //you can upload a single file or more 
    uploadTask(files) {
        ...
        [...files].forEach(file => {
            ...
            let formData = new FormData(); //API that reads the data  
            ...
            promises.push(this.ajax('/upload', 'POST', formData, () => {
                ...
                this.startUploadTime = Date.now(); //Date.now takes the milliseconds});
            }));
    ...
    formatTimeToHuman(milliseconds) {
    ...
    SELECT ALL- Reading info of the firebase with the forEach, listing the info received with json 
    SELECT one - 
    */
    ...
    connectFirebase() {
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyC1Ge5pxU0yhVqTqD8OjKlDXP40t_52fLU",
            authDomain: "dropbox-clone-45214.firebaseapp.com",
            databaseURL: "https://dropbox-clone-45214-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-45214",
            storageBucket: "dropbox-clone-45214.appspot.com",
            messagingSenderId: "715489487768",
            appId: "1:715489487768:web:71ee33e78019a3121d19ed",
            measurementId: "G-M54VQ4TNMQ"
        };
    ...
    readFiles() {
        ...
        //listens the reference, 'value' name of the event
        //snapshot -> photo of that event 
        this.getFirebaseRef().on('value', snapshot => {
            //'files' in database is a collection, which accepts forEach structure 
            ...
            snapshot.forEach(snapshotItem => {
                ...
                let data = snapshotItem.val(); //brings back the json with the info
    ...
    //dealing with the file format and it's icon 
    getFileView(file, key) {

        let li = document.createElement('li');

        li.dataset.key = key;
        li.dataset.file = JSON.stringify(file); //file is an object, but dataset only supports strings

        li.innerHTML = `
            ${this.getFileIconView(file.mimetype)} 
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('click', e => {
            ...
            if (e.shiftKey) {
                ...
                let firstLi = this.listFilesEl.querySelector('.selected'); //the first one clicked
                ...
                if (firstLi) {
                    ...
                    //  li.parentElement.childNodes brings an array of childs 
                    //li is the last element clicked
                    ...
                    lis.forEach((el, i) => {
                        ...
                        } //catching all elements between the first and last clicked
                    });
